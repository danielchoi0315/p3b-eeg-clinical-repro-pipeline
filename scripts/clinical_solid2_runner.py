#!/usr/bin/env python3
"""Clinical Solid2 pack runner.

Orchestrates:
- compile gate
- clinical data staging (if missing)
- participants->severity build
- event inspection + fail-closed mapping
...
def _stage_ensure_clinical_data(ctx: RunContext) -> Dict[str, Any]:
    ...
    if rc != 0:
        ...
        status = "FAIL"
        error = f"no dataset folders found in {ctx.clinical_bids_root}"
    elif not ready_dirs:
        status = "FAIL"
        error = "no dataset folders contain ready payload (participants + events + eeg files)"
...
def _stage_inspect_and_map(ctx: RunContext) -> Dict[str, Any]:
    stage = "inspect_and_map"
    started = time.time()
    log_path = ctx.audit_dir / f"{stage}.log"
    summary_path = ctx.audit_dir / f"{stage}_summary.json"
    ...
    ds_dirs = _discover_clinical_datasets(ctx.clinical_bids_root)
    if not ds_dirs:
        _write_json(summary_path, {"status": "FAIL", "reason": "no datasets discovered"})
        return _record_stage(
            ctx,
            stage=stage,
            status="FAIL",
            ...
            log_path=log_path,
            summary_path=summary_path,
            command="discover clinical datasets",
            error="no datasets",
        )

    mapping_payload: Dict[str, Any] = {"datasets": {}}
    status_rows: List[Dict[str, Any]] = []

    for d in ds_dirs:
        ds = d.name
        if not _dataset_payload_ready(d):
            reason = "dataset payload not ready (participants/events/eeg missing)"
            stop = ctx.clinical_pack_dir / f"STOP_REASON_{ds}.md"
            _write_stop_reason_dataset(
                stop,
                ds,
                reason,
                {
                    "dataset_path": str(d),
                    "participants_tsv_exists": (d / "participants.tsv").exists(),
                    "n_event_files": _count_event_files(d),
                    "n_eeg_files": _count_eeg_payload_files(d),
                },
            )
            status_rows.append({"dataset_id": ds, "status": "SKIP", "reason": reason, "stop_reason": str(stop), "mapping": {}})
            continue
        ...
        inspect_cmd = [
            ...
            ds,
            "--out_dir",
            str(ctx.clinical_pack_dir),
        ]
        # For clinical data we tolerate inspect script fail-closed return code
        # and rely on generated markdown + explicit mapping inference below.
        _run_cmd(inspect_cmd, cwd=REPO_ROOT, log_path=log_path)

        mapping, diag, reason = _infer_mapping_for_dataset(d)
        if mapping is not None:
            mapping_payload["datasets"][ds] = mapping
            status_rows.append({"dataset_id": ds, "status": "PASS", "reason": "", "mapping": mapping, "diag": diag})
        else:
            stop = ctx.clinical_pack_dir / f"STOP_REASON_{ds}.md"
            _write_stop_reason_dataset(stop, ds, reason, diag)
            status_rows.append({"dataset_id": ds, "status": "SKIP", "reason": reason, "stop_reason": str(stop), "diag": diag})

    # Write mapping config only for PASS datasets.
    ctx.clinical_event_map.parent.mkdir(parents=True, exist_ok=True)
    ctx.clinical_event_map.write_text(yaml.safe_dump(mapping_payload, sort_keys=True), encoding="utf-8")

    status = "PASS"
    if all(r["status"] != "PASS" for r in status_rows):
        status = "SKIP"
    ...
    _write_json(summary_path, {"status": status, "rows": status_rows, "mapping_status_json": str(mapping_status_json)})
    ...
    return _record_stage(
        ...
        log_path=log_path,
        summary_path=summary_path,
        command="inspect + infer mapping",
        outputs=[ctx.clinical_event_map, mapping_status_json],
    )
...
def _load_subject_group_for_dataset(severity_df: pd.DataFrame, dataset_id: str) -> pd.DataFrame:
    ...
    if df.empty:
        return df
    df["subject_id"] = df["subject_id"].astype(str).str.replace(r"^sub-", "", regex=True)
...
def _stage_extract_and_model(ctx: RunContext) -> Dict[str, Any]:
    ...
    for ds in pass_datasets:
        if time.time() > ctx.deadline_ts - 600:
...
def _stage_zip_bundle(ctx: RunContext) -> Dict[str, Any]:
    ...
    zpath = ctx.outzip_dir / "clinical_solid2_bundle.zip"
...
def _build_report(ctx: RunContext, run_status: str, run_error: str) -> Path:
    report = ctx.audit_dir / "CLINICAL_SOLID2_REPORT.md"
    ...
    lines.extend(
        [
            ...
            *res_rows,
            ...
            f"- Bundle: `{ctx.outzip_dir / 'clinical_solid2_bundle.zip'}`",
        ]
    )
...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument("--clinical_event_map", type=Path, default=Path("configs/clinical_event_map.yaml"))
...
def main() -> int:
    ...
    if args.out_root is None:
        ...
        out_root = Path("/filesystemHcog/runs") / f"{ts}_CLINICAL_SOLID2"
    else:
    ...
    if features_root_clinical is None:
        features_root_clinical = Path(f"/filesystemHcog/features_cache_CLINICAL_{run_ts}")
    ...
    print(f"BUNDLE_ZIP={ctx.outzip_dir / 'clinical_solid2_bundle.zip'}", flush=True)
