...
if [[ -z "$OUT_ROOT" ]]; then
  OUT_ROOT="/filesystemHcog/runs/$(date +%Y%m%d_%H%M%S)_NN_FINAL_PUSH"
fi
...
while [[ "$attempt" -le "$MAX_ATTEMPTS" ]]; do
  echo "[NN_FINAL_PUSH] attempt ${attempt}/${MAX_ATTEMPTS} out_root=${OUT_ROOT}"

  set +e
  cmd=(python scripts/nn_final_push_runner.py
    --out_root "$OUT_ROOT"
  ...
  # Minimal runtime-only fixes (no science/endpoint changes).
  log_blob="$(tail -n 800 "$OUT_ROOT"/AUDIT/*.log 2>/dev/null || true)"

  if echo "$log_blob" | rg -qi "VLEN strings|embedded NULL|HDF5|file locking"; then
  ...
  if [[ "$attempt" -eq 2 ]]; then
    # Keep retries deterministic; only reduce process fan-out.
    if [[ "$CPU_WORKERS" -gt 16 ]]; then
      CPU_WORKERS=16
    fi
  elif [[ "$attempt" -eq 3 ]]; then
    if [[ "$GPU_PARALLEL_PROCS" -gt 4 ]]; then
      GPU_PARALLEL_PROCS=4
    fi
  fi
...
report="$OUT_ROOT/AUDIT/NN_FINAL_PUSH_REPORT.md"
zip_path="$OUT_ROOT/OUTZIP/NN_SUBMISSION_PACKET.zip"

if [[ "$rc" -ne 0 ]]; then
  echo "[NN_FINAL_PUSH] failed after ${MAX_ATTEMPTS} attempts (rc=${rc})"
